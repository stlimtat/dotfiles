...
rsync -ar --progress ${HOME}/.{abnormal,aliases,bash_profile,bash_prompt,bashrc,curlrc,devenv,exports,extra,functions,gitconfig,ideavimrc,karabiner,wezterm.lua,wgetrc,zshrc} ${DOTFILES_DIR}/
rsync -ar --progress ${HOME}/bin/*.zsh ${DOTFILES_DIR}/bin/
rsync -ar --progress ${HOME}/bin/{wezterm,}.sh ${DOTFILES_DIR}/bin/
for s in .config/gh .config/lvim .config/nvim/lua .config/kitty .config/wezterm; do
  if [[ -d "${HOME}/${s}" ]]; then
